...
def flick(dt):
    ...
    global sleep_time, speed, ori, colormap
    ...
    if elapsed() > timer:
        if stim_cursor % 2 == 0:
            ...
            stim_cursor += 1
            speed = colormap[current_color_name][1]
            storeDataIntoFile(
...
def start(subject, trial, windowN):
    ...
    def on_draw():
        ...
        if current_color_name is "black":
            colormap['black'][0].blit(0, 0)
        else:
            colormap['white'][0].blit(0, 0)
        fps_display.draw()
    ...
    def drawer(this):
        def temp():
            ...
            this.clear()
            colormap[current_color_name][0].blit(x_pos, y_pos)
        return temp
...
def movingGrate(grating_dict,  # Dict{name: (grating, theta, L, moving_speed)}
    ...
    _, default_map = cf.generatePygletGrating(lambda x,y:(0,0,0,0), L=0, theta=0, title="")
    colormap = {}
    for (key, (grating, theta, L, moving_speed)) in grating_dict:
        themap, _ = cf.generatePygletGrating(grating, L=L, theta=theta, title=key)
        colormap[key] = (themap, moving_speed)
    colormap.update(default_map)

    speed = colormap[playlist[0][0]][1]
